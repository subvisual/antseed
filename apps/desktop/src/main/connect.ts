...
export interface ConnectDeps {
  ...
  /** Live funding-service state for the auto_deposit scope. Null when the buyer
   * proxy is unreachable or has no funding service. */
  getAutoDepositContext: () => Promise<ScopeContext['autoDeposit'] | null>;
...
async function handleConnectUrl(url: string, deps: ConnectDeps) {
  ...
  if (request.scopes.includes('auto_deposit')) {
    const autoDeposit = await deps.getAutoDepositContext();
    if (!autoDeposit) {
      deps.log?.('[connect] auto_deposit requested but funding context unavailable; aborting');
      return;
    }
    context = { autoDeposit };
  }

  const values = resolveScopeValues(request, identity.wallet, context);
